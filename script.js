(function() {
    ...
    function loadState() {
        try {
            ...
            if (saved) {
                ...
                if (!state.transactionPin) state.transactionPin = '1234';
                if (!state.dailyLimit) state.dailyLimit = 100.00;
                if (!state.todayWithdrawn) state.todayWithdrawn = 0.00;
                if (!state.lastWithdrawDate) state.lastWithdrawDate = getToday();
    ...
    function saveState() {
        try {
            const data = {
                ...
                lastWithdrawDate: state.lastWithdrawDate,
            };
    ...
    function renderBalance() {
        ...
        dailyLimitDisplay.textContent = formatK(state.dailyLimit);
        todayWithdrawnDisplay.textContent = formatK(state.todayWithdrawn);

        // Fee display
        const amount = parseFloat(withdrawAmount.value) || 0;
        const fee = calculateFee(amount);
        feeDisplay.innerHTML = `Fee: <strong>${formatK(fee)}</strong>`;
    ...
    function renderHistory() {
        ...
        sorted.forEach(w => {
            ...
            item.innerHTML = `
                    <div class="h-left">
                        <div class="h-amount">K${w.amount.toFixed(2)}${feeText}</div>
    ...
    function renderProofFeed() {
        ...
        proofCount.textContent = successes.length;
        let html = '';
        successes.forEach(w => {
            const dateStr = new Date(w.date).toLocaleDateString('en-US', {
                month: 'short',
    ...
    function processWithdrawal(amount, bankId, pin) {
        ...
        setTimeout(() => {
            // Deduct amount + fee
            state.balance -= totalDeduct;
            // Update today's withdrawn
            state.todayWithdrawn += amount;
            state.lastWithdrawDate = getToday();
            ...
            successMessage.textContent = `K${amount.toFixed(2)} + fee ${formatK(fee)} withdrawn successfully.`;
    ...
    function changePin() {
        ...
        pinHint.textContent = 'PIN updated successfully.';
        saveState();
    ...
    function openWallet() {
        ...
        pinHint.textContent = `Default: 1234 (current: ${state.transactionPin})`;
    ...
    $$('.tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btn.dataset.tab === 'settings') {
                dailyLimitInput.value = state.dailyLimit;
                pinHint.textContent = `Current PIN: ${state.transactionPin}`;
            }
        });
    });
    ...
    withdrawAmount.addEventListener('input', renderBalance);
    ...
    updateLimitBtn.addEventListener('click', updateDailyLimit);
